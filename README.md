...
This project focuses on the development of a low-cost, professional-grade monitoring system for industrial motors. Using an **ESP32**, we capture vibration, temperature and current signatures to feed three **Machine Learning** models (Python) that predicts mechanical or electrical failures before they occur. Additionally, the system implements SHapley Additive exPlanations (SHAP) to enhance the comprehension and transparency of the AI model's diagnostics.

*Este proyecto se enfoca en el desarrollo de un sistema de monitoreo profesional de bajo costo para motores industriales. Utilizando un **ESP32**, capturamos firmas de vibración y corriente para alimentar tres modelos de **Machine Learning** (Python) que predice fallas mecánicas o eléctricas antes de que ocurran. Adicionalmente, el sistema implementa SHapley Additive exPlanations (SHAP) para mejorar la compresión y transparencia de los diagnósticos en los modelos de IA *
...
* **Software & AI:** Python (Scikit-Learn), Node-RED, MQTT, SHapley Additive exPlanations.
